        ...
        try:
            ...
            if result['success']:
                ...
                if retry_count <= MAX_RETRY_ATTEMPTS:
                    ...
                    safe_execute_query('''
                        UPDATE payouts 
                        SET status = ?, retry_count = ?, scheduled_at = ?, 
                    ...
                    log_message('warning', f"Выплата {payout_id} не удалась, попытка {retry_count}/{MAX_RETRY_ATTEMPTS}")
    ...
    def process_expired_escrows(self):
        ...
        try:
            ...
            expired_escrows = safe_execute_query('''
                ...
                AND et.expires_at <= ?
                AND et.id NOT IN (
                    SELECT DISTINCT escrow_id FROM payouts WHERE escrow_id IS NOT NULL
                )
    ...
    def _create_automatic_payout(self, escrow: Dict):
        ...
        try:
            ...
            payout_id = safe_execute_query('''
                INSERT INTO payouts (
                    escrow_id, recipient_id, amount, status, trigger_type,
            ...
            safe_execute_query('''
                ...
                SET status = 'funds_released', released_at = ?
                WHERE id = ?
    ...
    def create_manual_payout(self, escrow_id: int, trigger_type: str = 'manual_release') -> Dict:
        ...
        try:
            ...
            existing_payout = safe_execute_query('''
                SELECT id FROM payouts WHERE escrow_id = ?
            ''', (escrow_id,), fetch_one=True)
        ...
        try:
            payout = safe_execute_query('''
                SELECT p.*, u.username, u.telegram_id,
                       et.offer_id, o.title as offer_title
                FROM payouts p
                JOIN users u ON p.recipient_id = u.id
...
def create_payout_tables():
    ...
    try:
        ...
        if cursor.fetchone():
            cursor.execute('''
                INSERT OR IGNORE INTO payouts_extended 
                (id, escrow_id, recipient_id, amount, status, transaction_id, created_at, completed_at)
                SELECT id, escrow_id, recipient_id, amount, status, transaction_id, created_at, completed_at
                FROM payouts
            ''')
