...
try:
    from payments_system import (
        register_payments_routes,
        create_payments_tables,
        PaymentManager,
        EscrowManager, MAX_RETRY_ATTEMPTS
)
    from telegram_webhooks import register_webhook_routes
...
try:
    from advanced_matching_algorithm import (
...
try:
    from analytics_api import add_analytics_routes, AnalyticsEngine
    from placement_tracking import PlacementTracker, add_tracking_routes
    from ai_recommendations import AIRecommendationEngine, add_ai_recommendation_routes
...
try:
    from notifications_system import notifications_bp, NotificationManager, AutoNotifications
...
def verify_payment_system():
    """Проверка готовности платежной системы"""
    try:
        ...
        for table in tables_to_check:
            result = safe_execute_query(f"SELECT COUNT(*) as count FROM {table}", fetch_one=True)
            if result is None:
                logger.warning(f"⚠️ Таблица {table} не найдена или недоступна")
                return False

        # Проверяем переменные окружения
        if not TELEGRAM_PAYMENT_TOKEN:
            logger.warning("⚠️ TELEGRAM_PAYMENT_TOKEN не настроен - платежи через Telegram недоступны")
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🚀 TELEGRAM MINI APP - ЗАПУСК ПРИЛОЖЕНИЯ")
    print("="*60)

    # Информация о конфигурации
    print(f"📱 Bot Token: {'✅ Настроен' if BOT_TOKEN else '❌ НЕ НАСТРОЕН!'}")
    print(f"🗄️ Database: SQLite ({DATABASE_PATH})")
    print(f"👤 Your Telegram ID: {YOUR_TELEGRAM_ID}")
    print(f"🔗 Telegram Integration: {'✅ Активна' if TELEGRAM_INTEGRATION else '❌ КРИТИЧЕСКАЯ ОШИБКА!'}")
    print(f"🤖 Telegram Service: {'✅ Готов' if telegram_service else '❌ КРИТИЧЕСКАЯ ОШИБКА!'}")
    print(f"🎯 Offers System: {'✅ Активна' if OFFERS_SYSTEM_ENABLED else '❌ Отключена'}")
    print(f"💬 Responses System: {'✅ Активна' if RESPONSES_SYSTEM_ENABLED else '❌ Отключена'}")
    # Инициализируем продвинутый алгоритм
    advanced_matching_available = initialize_advanced_matching()
    print(f"🧠 Advanced Matching: {'✅ Активен v2.0' if advanced_matching_available else '❌ Недоступен'}")

    # Инициализируем систему аналитики Этапа 3
    analytics_initialized = initialize_full_analytics_system()
    print(f"📊 Analytics System: {'✅ Готова' if analytics_initialized else '❌ Частично доступна'}")

    # Проверка критических требований
    if not BOT_TOKEN:
        print("\n❌ КРИТИЧЕСКАЯ ОШИБКА: BOT_TOKEN не настроен!")
        print("Установите BOT_TOKEN в .env файле")
        sys.exit(1)

    if not TELEGRAM_INTEGRATION:
        print("\n❌ КРИТИЧЕСКАЯ ОШИБКА: Telegram интеграция недоступна!")
        print("Установите зависимости: pip install aiohttp asyncio")
        sys.exit(1)
    ...
    try:
        ...
        if init_database():
            ...
            print("❌ Проблема с базой данных. Запустите sqlite_migration.py")
            print("\nДля создания базы данных выполните:")
            print("python sqlite_migration.py")
            sys.exit(1)

        # Проверка платежной системы
        payment_system_ready = verify_payment_system()
        print(f"💳 Payment System: {'✅ Готова' if payment_system_ready else '⚠️ Частично доступна'}")
        ...
        if RESPONSES_SYSTEM_ENABLED:
            print("  • Подходящие офферы для канала: http://localhost:5000/channel-offers/{channel_id}")
        print("  • API Test: http://localhost:5000/test")
        print("  • Health Check: http://localhost:5000/health")
        print("  • Database Info: http://localhost:5000/api/database-info")
        print("\n🔒 БЕЗОПАСНОСТЬ:")
        print("  • Строгая фильтрация каналов по владельцам")
        print("  • Rate limiting активен")
        print("  • SQL injection защита")
        print("  • Логирование безопасности")
        print("\n🛑 Для остановки нажмите Ctrl+C")
        # Инициализация системы аналитики
        if analytics_initialized:
            print("🎉 Все системы аналитики готовы к работе!")
            print("\n📊 ДОСТУПНЫЕ ФУНКЦИИ:")
            print("  • Дашборд аналитики: http://localhost:5000/analytics")
            print("  • Продвинутые офферы: http://localhost:5000/advanced-offers/{channel_id}")
            print("  • API метрик: http://localhost:5000/api/analytics/*")
            if PLACEMENT_TRACKING_ENABLED:
                print("  • Отслеживание: http://localhost:5000/api/placements/*")
            if AI_RECOMMENDATIONS_ENABLED:
                print("  • AI-рекомендации: http://localhost:5000/api/ai/*")
        else:
            print("⚠️ Система аналитики частично доступна")

        if not init_database():
            print("❌ Ошибка инициализации базы данных")
            sys.exit(1)

        # Настройка системы выплат
        if PAYOUT_SYSTEM_ENABLED:
            setup_payout_routes()

            # Автозапуск планировщика (опционально)
            try:
                payout_manager.start_auto_payouts()
                logger.info("🚀 Планировщик автоматических выплат запущен")
            except Exception as e:
                logger.error(f"❌ Ошибка автозапуска планировщика: {e}")

            print("\n💰 СИСТЕМА ВЫПЛАТ АКТИВНА:")
            print("  • Панель управления: http://localhost:5000/payouts-dashboard")
            print("  • API выплат: http://localhost:5000/api/payouts/*")
            print("  • Автоматические выплаты: Включены")
        else:
            print("\n⚠️ Система выплат отключена")

        # Запуск Flask приложения
        app.run(host='0.0.0.0', port=5000, debug=False)  # debug=False для продуктивного режима

    except Exception as e:
        print(f"❌ Ошибка запуска: {e}")
        print("\n📋 Проверьте:")
        print("1. База данных SQLite создана (запустите sqlite_migration.py)")
        print("2. BOT_TOKEN настроен в .env файле")
        print("3. Зависимости установлены: pip install -r requirements.txt")
        print("4. Файлы add_offer.py и offer_responses.py существуют")
        print("5. Файл real_telegram_integration.py существует")
        print(f"\nПуть к базе данных: {DATABASE_PATH}")
        print("\nДля создания базы данных:")
        print("python sqlite_migration.py")
        if ANALYTICS_SYSTEM_ENABLED:
            print("\n📊 АНАЛИТИКА И ОТЧЕТЫ:")
            print("  • Дашборд аналитики: http://localhost:5000/analytics")
            print("  • Статус системы: http://localhost:5000/api/analytics/system-status")
            print("  • Комплексные данные: http://localhost:5000/api/analytics/dashboard-data")

            if PLACEMENT_TRACKING_ENABLED:
                print("  • Отслеживание размещений: /api/placements/*")

            if AI_RECOMMENDATIONS_ENABLED:
                print("  • AI-рекомендации: /api/ai/recommendations")
        if payment_system_ready:
            print("💰 Доступные функции платежей:")
            print("  • Эскроу-транзакции")
            print("  • Автоматические выплаты")
            print("  • Интеграция с Telegram Wallet")
            print("  • Система споров")

        print("\n🌐 Новые страницы:")
        print("  • Платежи: http://localhost:5000/payments")
        print("  • API навигации: http://localhost:5000/api/navigation/menu")