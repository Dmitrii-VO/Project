...
class TelegramAuth:
    """Класс для работы с аутентификацией Telegram"""

    @staticmethod
    def get_current_user_id() -> Optional[str]:
        """Получение ID текущего пользователя Telegram"""
        # Проверяем заголовок (основной способ)
        ...
        # Проверяем параметры запроса (только в режиме отладки)
    ...
    def validate_telegram_data(init_data: str) -> bool:
        ...
        try:
            # Здесь должна быть реализована проверка подписи Telegram
            ...
            # В реальном приложении используйте токен бота
            ...
            current_app.logger.error(f"Error validating Telegram data: {e}")
            return False
